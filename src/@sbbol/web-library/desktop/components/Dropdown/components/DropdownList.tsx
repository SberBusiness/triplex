...
export class DropdownList extends React.Component<IDropdownListProps, IDropdownListState> {
    ...
    public componentDidUpdate(prevProps: IDropdownListProps): void {
        const {dropdownOpened} = this.props;
    ...
    public render(): JSX.Element {
        ...
        return (
            <div className={classnames('cssClass[dropdownList]', className)} role="listbox" ref={this.containerRef} {...htmlDivAttributes}>
                {this.renderChildren()}
            </div>
        );
    }
    ...
    private scrollContainerToItem = (itemIndex: number) => {
        ...
        if (parent && activeItem) {
            ...
            if (parentTop > itemTop) {
                parent.scrollTop = parent.scrollTop - parentTop + itemTop;
            } else if (itemBottom > parentBottom) {
                // Item ниже нижней границы parent.
                parent.scrollTop = parent.scrollTop + itemBottom - parentBottom;
            }
        }
    };

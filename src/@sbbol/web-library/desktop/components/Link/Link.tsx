...
export const Link = React.forwardRef<HTMLAnchorElement, TLinkProps>(
    ({className, linkType, size, children, onMouseDown, onBlur, ...rest}, ref) => {
        const [focusedByClick, setFocusedByClick] = useState(false);

        const contentBefore = linkType === ELinkType.TEXT ? rest.contentBefore : undefined;
        const contentAfter = linkType === ELinkType.TEXT ? rest.contentAfter : undefined;

        if (linkType === ELinkType.TEXT) {
            const {contentBefore, contentAfter, ...restProps} = rest;
            rest = restProps;
        }

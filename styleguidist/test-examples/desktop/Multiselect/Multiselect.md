...
const renderDropdown = (dropdownProps) => {
    ...
    return (
        <Multiselect.Dropdown {...dropdownProps} topOrientation={topOrientation}>
            <Multiselect.Dropdown.Header>
                <Input
                    setInputRef={inputFilterNode}
                    value={filter}
                    onChange={(e) => {
...
<>
    ...
    <ComponentPreview
        style={{boxSizing: 'border-box', height: open ? '370px' : undefined, paddingTop: open && topOrientation ? '335px' : undefined}}
    >

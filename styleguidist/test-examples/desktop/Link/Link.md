...
const renderLink = () => <Link linkType={linkType} size={linkSize} href="javascript:void(0)" ref={linkRef} contentBefore={contentBefore} contentAfter={contentAfter}>Текст ссылки</Link>;

<>
    <ComponentOptions checkboxOptions={checkboxOptions} inputOptions={inputOptions} />
    <ComponentPreview style={{paddingLeft: '10px'}}>
        {type === 'line' ? (
            <ExampleBackground background={ExampleBackgroundColor['graphite-01']}>
                {renderLink()}
            </ExampleBackground>
        ) : renderLink()}
    </ComponentPreview>

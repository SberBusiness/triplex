...
const MAX_ROW_LENGTH = 352;

const [checkedRows, setCheckedRows] = useState([]);
// Выбраны все элементы таблицы(даже те, которые не отображаются).
    ...
    return (
        <Checkbox
            ...
            onChange={(event) => {
                ...
                setCheckedRows((rows) => {
                    if (event.target.checked) rows.push(rowKey);
                    else rows.splice(checkedRows.indexOf(rowKey), 1);
                    return [...rows];
                });
            }}
    ...
    return (
        <Checkbox
            ...
            onChange={(event) => {
                ...
                setCheckedRows((rows) => {
                    if (event.target.checked || bulk) rows = data.map((row) => row.rowKey);
                    else rows.length = 0;
                    return [...rows];
                });
            }}
...
const selectAll = () => {
    ...
    setCheckedAllItems((checkedAllItems) => !checkedAllItems);
};
    ...
    <MasterTable.TableFooter data-test-id="TestTable__MasterTable.TableFooter">
        <MasterTable.TableFooter.Summary data-test-id="TestTable__MasterTable.TableFooter.Summary">
            ...
            <MasterTable.TableFooter.Summary.SelectedCount>
                {`Выбрано ${checkedAllItems ? MAX_ROW_LENGTH : checkedRows.length} из ${MAX_ROW_LENGTH}`}
            </MasterTable.TableFooter.Summary.SelectedCount>
            <MasterTable.TableFooter.Summary.SelectAllButton onClick={selectAll}>
                {checkedAllItems ? 'Сбросить всё' : `Выбрать все ${MAX_ROW_LENGTH}`}
